...
if nav_choice == "Home":
    ...
    # Define a function to scale input features
    ...
    valid_input = (0 <= age_years <= 120 and 
                ...
                40 <= diastolic_bp <= 150)
    if submit:
        if all([0 <= age_years <= 120, 30 <= weight <= 200, 100 <= height <= 250, 40 <= systolic_bp <= 250, 80 <= diastolic_bp <= 150]):
            # Categorizations
            ...
            try:
                ...
                # Predict health risk
                health_risk_prediction = predictive_model.predict(scaled_features)
